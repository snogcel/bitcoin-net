...
module.exports = function (_EventEmitter) {
  ...
  function PeerGroup(params, opts) {
    ...
    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(PeerGroup).call(this));
    ...
    _this._txPool = [];
    _this._txPoolMap = {};
    _this._txPoolPrevLength = 0;

    var wrtc = opts.wrtc || getBrowserRTC();
    _this._exchange = exchange(params.magic.toString(16), { wrtc: wrtc });
    _this._exchange.on('error', _this._error.bind(_this));
    _this._exchange.on('peer', function (peer) {
      if (!peer.incoming) return;
      _this._onConnection(null, peer);
    });
  ...
  _createClass(PeerGroup, [{
    ...
    value: function _onConnection(err, socket) {
      ...
      var opts = Object.assign({ socket: socket }, this.peerOpts);
    ...
    value: function _connectDNSPeer(cb) {
      ...
      dns.resolve(seed, function (err, addresses) {
        ...
        _this3._connectTCP(address, _this3._params.defaultPort, cb);
      });
    }
    ...
    value: function _connectWebSeeds() {
      ...
      try {
        for (var _iterator = this._params.webSeeds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var seed = _step.value;

          if (typeof seed === 'string') {
            var url = utils.parseAddress(seed);
            var port = url.port || this._params.defaultWebPort || DEFAULT_PXP_PORT;
            seed = { transport: 'websocket', address: url.hostname, opts: { port: port } };
          }
    ...
    value: function send(command, payload, assert) {
      ...
      try {
        for (var _iterator2 = this.peers[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
    ...
    value: function connect() {
      ...
      if (this._connectWeb && this._params.webSeeds && this._params.webSeeds.length) {
        var nSeeds = Math.max(1, Math.min(this._params.webSeeds.length, Math.floor(this._numPeers / 2)));
        var i = 0;
        var onPeer = function onPeer() {
          i++;
          if (i < nSeeds) return;
          _this4.removeListener('peer', onPeer);
          _this4._fillPeers();
        };
        this.on('peer', onPeer);
    ...
    value: function close(cb) {
      ...
      this.unaccept(function (err) {
        if (err) return cb(err);
        var peers = _this5.peers.slice(0);
        ...
        try {
          for (var _iterator3 = peers[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            ...
            peer.once('disconnect', function () {
              if (_this5.peers.length === 0) cb(null);
            });
    ...
    value: function accept(port, cb) {
      ...
      this._exchange.accept('websocket', { port: port }, function (err) {
        if (err) return cb(err);
        _this6._exchange.accept('webrtc', function (err) {
          // ignore errors about not having a webrtc transport
          if (err && err.message === 'Transport "webrtc" not found') err = null;
          if (err) return _this6.unaccept(function () {
            return cb(err);
          });
          _this6.accepting = true;
          cb(null);
        });
      });
    }
    ...
    key: 'unaccept',
    value: function unaccept(cb) {
      var _this7 = this;

      if (!this.accepting) return cb(null);
      this._exchange.unaccept('websocket', function (err1) {
        _this7._exchange.unaccept('webrtc', function (err2) {
          _this7.accepting = false;
          if (cb) return cb(err1 || err2);
          if (err1 || err2) return _this7._error(err1 || err2);
        });
      });
    }
    ...
    value: function addPeer(peer) {
      ...
      var onMessage = function onMessage(message) {
        _this8.emit('message', message, peer);
        _this8.emit(message.command, message.payload, peer);
      };
      peer.on('message', onMessage);

      peer.on('tx', function (tx) {
        var hash = tx.getHash().toString('base64');
        if (!_this8._txPool[hash]) {
          _this8._txPoolMap[hash] = tx;
          _this8._txPool.push(tx);
        }
      });

      peer.once('disconnect', function (err) {
        var index = _this8.peers.indexOf(peer);
        _this8.peers.splice(index, 1);
        peer.removeListener('message', onMessage);
        debug('peer disconnect, peer.length = ' + _this8.peers.length + ', reason=' + err + '\n' + err.stack);
        if (_this8.connecting) _this8._fillPeers();
        _this8.emit('disconnect', peer, err);
      });
      peer.on('error', function (err) {
        _this8.emit('peerError', err);
        peer.disconnect(err);
      });
    ...
    key: 'getTransactions',
    value: function getTransactions(blockHash, txids, cb) {
    ...
    value: function _request(method) {
      ...
      args.push(function (err, res) {
        ...
        if (err && err.timeout) {
          ...
          peer.disconnect(err);
          _this9.emit('requestError', err);
          return _this9._request.apply(_this9, _arguments);
        }
        cb(err, res, peer);
      });
    ...
    value: function _clearTxPool() {
      ...
      try {
        for (var _iterator4 = removed[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          var tx = _step4.value;
